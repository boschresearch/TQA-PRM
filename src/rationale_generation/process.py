...
def get_majority_vote_responses(args, model, messages, tokenizer, assistant_response, flag, must_yes_no=False):
    ...
    current_conversations = [deepcopy(messages) for _ in range(majority_num)]
...
def generate_critic_prompt(problem: str, steps: list, ref=None) -> str:
    ...
    for idx, step in enumerate(steps):
        content = re.sub(r'^Step \d+:\s*', '', step) #delete the "Step"
    ...
    if ref is None:
        #prompt_template = f'The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the logical and factual correctness of each solution paragraph. ``Correct`` means accurate table lookups, valid reasoning steps, and no miscalculations.  Split your verification by `### Paragraph {{ID}}`. \n\nYour verification for each paragraph should be constructed by 2 parts, wrapped by `<analyze></analyze>`,  and `<output></output>` separately.\n\n1. in `<analyze>` part, address these five aspects:\nPart 1: **Restatement**: Verbalize the claim, statement, or answer this paragraph makes.\nPart 2. **Data Check**: traverse the table and quote row and column to show where the evidence comes from.\nPart 3.  **Logical Consistency**: Check if the claim is logically valid.\nPart 4.  **Numeric Accuracy**: Check if calculations such as sums, percentages, and table lookups, are computed correctly.\nPart5. **Format**: Verify that the final paragraph puts the final answer using \\boxed{{Final Answer}}. If any aspect is wrong, then the paragraph is wrong.\n2. In `<output>` part, judge if this paragraph is correct or incorrect and put the answer into `<output>` part, e.g., ``<output>\n**Judgement**: $\\boxed{{Yes/No}} <\output>``. Every Paragraph must have an `<output>` part.'
        ...
        #prompt_template = f'The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the correctness of paragraph in the solution. Split your verification by `### Paragraph {{ID}}`. \n\nYour verification for each paragraph should be constructed by 3 parts, wrapped by `<rollout></rollout>`, `<analyze></analyze>` and `<output></output>` separately.\n\n1. in `<rollout>` part, starting from current paragraph, traverse the table and generate a coherent, step-by-step sequence of follow-up steps until you arrive at a final answer. Present that final answer at the end as: \\boxed{{Final Answer}}.\n2. in `<analyze>` part:\n- **Compare rollouts**: Look at the current `<rollout>` and the previous one (if any). \n- **Explain inferences**: Explain reasoning steps in your new rollout.\n- **Resolve conflicts**: If the boxed final answers differ, state which one is correct and why.\n- **Judge the paragraph**: Based on the above, analyze in detail whether current paragraph  is correct or incorrect.\n3. In `<output>` part, judge if this paragraph is correct or incorrect and put the answer into `<output>` part, e.g., ``<output>\n**Judgement**: $\\boxed{{Yes/No}} <\output>``.'
        #prompt_template = f'''The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the logical and factual correctness of each solution paragraph. Split your verification by `### Paragraph {{ID}}`. \n\nStructure your analysis into four tagged sections:\n\n<affirmative>\nLet's assume this paragraph is correct, demonstrate why:\n1. **Restatement**: Rephrase the question and this paragraph's claim or answer.\n2. **Data Check**: Show the exact table rows and columns that back up the claim in details.\n3. **Logical Consistency**: Explain how the reasoning follows valid, step-by-step logic.\n4. **Numeric Accuracy**: Confirm that any sums, percentages, dates, or lookups are computed without error.\n5. **Format Verification**: Verify that the final paragraph concludes with `\\boxed{{Final Answer}}`.\n</affirmative>\n\n<negative>\nLet's ignore what <affirmative> said. Assuming this paragraph is incorrect, identify flaws:\n1. **Restatement Omission/Error**: Where the rephrasing misrepresents the question or claim.\n2. **Data Mismatch**: Any table entries that contradict or fail to support the claim.\n3. **Logical Gap**: Any leap in reasoning or invalid inference.\n4. **Calculation Error**: Mistakes in arithmetic, dates or lookup values.\n5. **Format Issue**: Missing or misplaced in final paragraph `\\boxed{{Final Answer}}`.\n</negative>\n\n<referee>\nLet's weigh the strengths of the <affirmative> and <negative> sections point by point. Then decide in details which perspective wins</referee>\n\n<output>\n**Judgement**: $\\boxed{{Yes}} (if the affirmative case outweighs the negative) or \n**Judgement**: $\\boxed{{No}} (if the negative case wins)\n</output>'''
        #prompt_template = f'''The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the logical and factual flawness or correctness of each solution paragraph. Split your verification by `### Paragraph {{ID}}`. \n\nStructure your analysis into four tagged sections:\n\n<affirmative>\nLet's assume this paragraph is correct, analyze the current paragraph in details.\n</affirmative>\n\n<negative>\nLet's ignore what <affirmative> section said. You Must assume this paragraph is incorrect, and MUST identify the flaws of this step\n</negative>\n\n<referee>\nLet's weigh the strengths of the <affirmative> and <negative> sections point by point. Then decide in details which perspective wins</referee>\n\n<output>\n**Judgement**: $\\boxed{{Yes}} (if the affirmative case outweighs the negative) or \n**Judgement**: $\\boxed{{No}} (if the negative case wins)\n</output>'''
        #prompt_template = f'The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the correctness of paragraphs in the solution. Split your verification by `### Paragraph {{ID}}`. \n\nYour verification for each paragraph should be constructed by 4 parts, wrapped by `<rephrase></rephrase>`, `<react></react>`, `<analyze></analyze>` and `<output></output>` separately.\n\n1. In `<rephrase>` part,  rephrase the paragraph as a clear, self-contained question, preserving all information from the original paragraph.\n\n2. In `<react>` part, use alternating steps of **Thought**, **Action**, **Observation** to systematically solve the problem from <rephrase> part.\n\n**Thought**: Based on the information currently available, reason through the problem and determine the goal of the next action.\n\n**Action**: Each action must be one of the following five types:\n1. LOOKUP_ROW[row] – Find a row in the table whose label matches the given text.\n2. LOOKUP_COLUMN[column] – Find a column in the table whose header matches the given text.\n3. READ_CELL[row, column] – Retrieve the value in the specified row and column.\n4. COMPUTATION(values): Perform arithmetic or other clearly defined mathematical operations on the provided values.\n5. Finish[answer]: Once a clear answer has been determined, use this action to return the answer and terminate the task.\n\n**Observation**: Record the factual result of the action.\n\n3. in `<analyze>` part, based on <rephrase> and <react> results, analyze in detail if the current paragraph is correct or incorrect, citing the relevant evidence.\n\n4. In `<output>` part, make a final judgement and put the judgement into `<output>` part, e.g., ``<output>\n**Judgement**: $\\boxed{{Yes/No}} </output>``.'
        #prompt_template = f'The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the correctness of paragraph in the solution.  Split your verification by `### Paragraph {{ID}}`. \n\nYour verification for each paragraph should be constructed by 3 parts, wrapped by `<verify></verify>`, `<analyze></analyze>` and `<output></output>` separately.\n\n1. in `<verify>` part, you must write **Python code** in the form of ```python\n{{CODE}}\n``` to verify every details in the current paragraph that can be verified by code and compute the final answer to the [Table Question Answering Problem]:\n\n{problem}. You must import Pandas to convert the table into a Pandas DataFrame, verify every details in the paragraph, and compute the final answer to the [Table Question Answering Problem]. Make sure to print the critic results in the code. Every code will be executed automatically by system. You need to analyze the `[Code Output]` after code executing.\n\n Pay attention that you must follow the format of ```python\n{{CODE}}\n``` when you write the code, otherwise the code will not be executed.\n2. in `<analyze>` part, you need to analyze the reasoning process and explain why the paragraph is correct or incorrect in detail.\n3. In `<output>` part, judge if this paragraph is correct or incorrect and put the answer into `<output>` part, e.g., ``<output>\n**Judgement**: $\\boxed{{Yes/No}} </output>``. Every Paragraph must have an `<output>` part.'
        prompt_template = f'The following is the table question answering problem and a solution (split into paragraphs, enclosed with tags and indexed from 1):\n[Table Question Answering Problem]\n\n{problem}\n\n[Solution]\n\n{solution_section}\n\nYour task is to verify the correctness of paragraph in the solution.  Split your verification by `### Paragraph {{ID}}`. \n\nYour verification for each paragraph should be constructed by 2 parts, wrapped by `<analyze></analyze>` and `<output></output>` separately.\n\n1. In `<analyze>` part, you need to analyze the reasoning process and explain why the paragraph is correct or incorrect in detail.\n2. In `<output>` part, judge if this paragraph is correct or incorrect and put the answer into `<output>` part, e.g., ``<output>\n**Judgement**: $\\boxed{{Yes/No}} </output>``. Every Paragraph must have an `<output>` part.'
    else:
...
def process_json_file(args, filename, data_dir, conversation_dir, model, tokenizer):
    ...
    for group in steps_groups:
        ...
        if len(group) == 0:
            ...
            #sys_prompt = 'You are a table question answering expert. Your task is to review and critique the paragraphs in solution step by step with python code.'
            sys_prompt = 'You are a table question answering expert. Your task is to review and critique the paragraphs in solution step by step.'
            messages = [
