...
def load_data(args):
        ...
        for s in splits:
            data_path = os.path.join(args.base_path, args.data_dir, args.data_name)
            with open(data_path, "r") as f:
                data = [json.loads(line) for line in f]

            for idx, item in tqdm(enumerate(data), desc=f"Processing {s}"):
                meta_data = {}
                for k, v in item.items():
                    if k not in ['answer','label','statement','table_caption', 'table_text','text']:
                        meta_data[k] = v

                table_content = "Table is: " + str(item['table_text'])+ "\n"
                question_content= "Question is: " + str(item['statement'])
                if "table_caption" in item.keys():
                    table_caption = "Table Caption is: " + str(item['table_caption']) + "\n"
                    problem = table_caption + table_content + question_content
                elif "text" in item.keys():
                    table_caption = "Table Caption is: " + str(item['text']) + "\n"
                    problem = table_caption + table_content + question_content
                else:
                    problem = table_content + question_content
                    
                if "answer" in item.keys():           
                    extracted_groundtruth = item['answer']
                    
                elif "label" in item.keys():
                    extracted_groundtruth = item['label']
