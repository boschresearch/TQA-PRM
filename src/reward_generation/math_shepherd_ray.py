...
from utils.eval import check_correctness
...
class Generator:
    ...
    def generate_step_by_step_paths(self, problem, num_samples_list, temperature_list):
        ...
        #bugs_path = os.path.join("/home/tal1rng/GenPRM/src/bugs", "bugs_tab.json")
        #save_output = {
         #   "messages": messages,
    ...
    def process_inference_paths(self, paths, expected_answer, correct_paths, incorrect_paths, invalid_idxs):
        ######process the above step-by-step paths and divide them into correct, incorrect, and invalid groups
        for path_idx, path in enumerate(paths):
            ...
            #     invalid_idxs.append(1)
                ...
                if check_correctness(final_step, expected_answer, self.args):
    ...
    def evaluate_inference_step(self, problem, extracted_groundtruth, inference_steps: List[str], step_idx: int, num_mc: int=25) -> float:
        ##########Rollout to evaluate the intermediate step of one path
        ...
        if step_idx == len(inference_steps) - 1:  # Last step, no completion required, just check the answer
            ...
            if check_correctness(generated_response, extracted_groundtruth, self.args):  # Check correctness of the last step
            ...
            for idx in range(num_mc):
                ...
                if check_correctness(final_step, extracted_groundtruth, self.args):  # Check correctness of the response
                    correct_count += 1
    ...
    def evaluate_inference_paths(self, problem, extracted_groundtruth, inference_paths, data, start_path_idx=0, num_mc=25):
        #######evaluate each step of each inference path
        all_scores = []
        if start_path_idx > 0:
    ...
    def evaluate_problem(self, data):
        ...
        if not self.args.only_monte_carlo:
            ...
            """while 0 <= len(correct_paths) < half_sample:
                #default num_try=126
                #try max 126 times to sample 16 paths every time
                ...
                if try_cnt >= self.args.num_try:
                    break
            try_cnt = 0
            while 0 <= len(incorrect_paths) - sum(invalid_idxs) < half_sample: ###生成很多paths可能是因为这行代码导致的
                ### filtering out the invalid paths so you only count the usable incorrect ones
                more_inference_paths = self.generate_step_by_step_paths(problem, num_samples_list=[16], temperature_list=[0.7])
                random.shuffle(more_inference_paths)
                correct_paths, incorrect_paths, invalid_idxs = self.process_inference_paths(
                    more_inference_paths, extracted_groundtruth, correct_paths, incorrect_paths, invalid_idxs
            ...
            metrics['correct_num'] += len(correct_paths) #total sampled correct paths
            metrics['incorrect_num'] += len(incorrect_paths) #total sampled incorrect paths
            metrics['invalid_num'] += sum(invalid_idxs)
            metrics['base_score'] = 1.0 * metrics['correct_num'] / (metrics['correct_num'] + metrics['incorrect_num'])
            data['metrics'] = metrics
             ...
             #   data['flag'] = 'hard'
            if not self.args.close_monte_carlo:
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='_output_TOT_v5_granularity', help="Path to save the output JSON file.")
    parser.add_argument('--add_sleep', '-s', action='store_true', help="Reverse the order of inference steps.")
    parser.add_argument('--num_try', type=int, default=64, help="Number of completions to generate per inference step.")
    ...
    cot_prompt_dict = {
        ...
        #'default2': """Please inspect the table(s) and then provide an Answer to the question.  Please reason step by step. Attention: You MUST put your final answer using the following format: Final answer: \\boxed{final answer}, Attention: You MUST put your final answer using the following format: Final answer: \\boxed{final answer}.""",
        'default':"""Please reason step by step by traversing the table(s) to answer the question, \nNote that:\n1. Traverse the table row by row, visiting each row once and only once. \n2. At each step, clearly describe what you observe or deduce from that row and record the intermediate answer to the question. \n3. You MUST put your final answer using the following format: Final answer: \\boxed{final answer}.\nNote that:\n1. Traverse the table row by row, visiting each row once and only once. \n2. At each step, clearly describe what you observe or deduce from that row and record the intermediate answer to the question. \n3. You MUST put your final answer using the following format: Final answer: \\boxed{final answer}""",
        #'default': """Please inspect the table(s) and answer the question, reasoning step by step: \n Step 1. Count all rows (including the header). \nStep 2. Identify and select the relevant rows. \nSubsequent Steps: Analyze each selected row, one row per step.\nFor example:\nStep 1. Let’s count the rows: … In total, the table has 23 rows.\nStep 2. Rows 2, 7, and 16 appear most useful, so we select them.\nStep 3. Analyze row 2.\nStep 4. Analyze row 7.\nStep 5. Analyze row 16.\nStep6.Final answer: \\boxed{your answer}""",
        'tabfact2': """Please inspect the table(s) and then provide a True or False to the question.  Please reason step by step. Attention: You MUST put your final answer using the following format: Final answer: \\boxed{final answer(True/False)}, Attention: You MUST put your final answer using the following format: Final answer: \\boxed{final answer(True/False)}.""",
        "tabfact":"""Please reason step by step by traversing the table(s) to answer the question, \nNote that:\n1. Traverse the table row by row, visiting each row once and only once. \n2. At each step, clearly describe what you observe or deduce from that row and record the intermediate answer to the question. \n3. You MUST put your final answer using the following format: Final answer: \\boxed{True/False}.\nNote that:\n1. Traverse the table row by row, visiting each row once and only once. \n2. At each step, clearly describe what you observe or deduce from that row and record the intermediate answer to the question. \n3. You MUST put your final answer using the following format: Final answer: \\boxed{True/False}"""
    }
