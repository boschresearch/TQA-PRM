...
class GenPRM:
    def __init__(self, model_path, tensor_parallel_size=1,lora=False, lora_path=None):
    ...
    def _single_inference(
        ...
        analyze_template="<analyze>\nLet's analyze the Paragraph {cur_step} step by step: ",
        verify_template="<verify>\nLet's use python code to find any potential error:\n```python\n",
        output_template="<output> Please respond with exactly Yes or No. \n**Judgement**: $\\boxed",
        logging=True
    ):
        ...
        if analyze:
            ...
            if self.lora:
                output1 = self.model.generate(prompt + analyze_start, 
                                                sampling_params=sampling_params, 
                                                use_tqdm=False,
                                                lora_request=LoRARequest("wtq_lora",1,self.lora_path))[0].outputs[0]
            else:
        ...
        while len(cur_prompts) > 0:
            ...
            if left_tokens > 0 and cur_time < time_limit:
                ...
                if self.lora:
                    output2 = self.model.generate(prompt + cur_prompts[0], 
